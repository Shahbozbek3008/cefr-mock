...
const tones: Record<'light' | 'dark', Tone> = {
  light: { ink: (alpha) => `rgba(20,22,30,${alpha})`, iosInk: '#14161E', iosScale: 1 },
  dark: { ink: (alpha) => `rgba(0,0,0,${Math.min(1, alpha * 2.5)})`, iosInk: '#000000', iosScale: 1.8 },
};

const boxShadow = (layers: Layer[]): ViewStyle =>
  ({
    boxShadow: layers.map((l) => `${l.x}px ${l.y}px ${l.blur}px ${l.spread}px ${l.color}`).join(', '),
  }) as ViewStyle;

const ios = (color: string, opacity: number, blur: number, y: number): ViewStyle => ({
  shadowColor: color,
  shadowOpacity: Math.min(1, opacity),
  shadowRadius: blur,
...
export const createElevation = (colors: Colors, scheme: 'light' | 'dark') => {
  const { ink, iosInk, iosScale } = tones[scheme];
  const line = colors.hairline;
  const neutral = (opacity: number, blur: number, y: number) => ios(iosInk, opacity * iosScale, blur, y);
  ...
  return {
    ...
    segment: shadow(
      [
        ...
        { x: 0, y: 0, blur: 0, spread: 0.5, color: line },
      ],
      neutral(0.08, 1, 1),
    ),
    ...
    card: shadow(
      [
        { x: 0, y: 0, blur: 0, spread: 1, color: line },
    ...
    hero: shadow([{ x: 0, y: 16, blur: 32, spread: -16, color: 'rgba(46,72,126,0.55)' }], ios('#2E487E', 0.55, 16, 16)),
    ...
    warningRing: {
      outlineWidth: 3,
      outlineStyle: 'solid',
      outlineColor: colors.warning.bg,
    } as ViewStyle,

    thumb: shadow([{ x: 0, y: 1, blur: 3, spread: 0, color: 'rgba(0,0,0,0.2)' }], ios('#000000', 0.2, 3, 1)),
  };
};
